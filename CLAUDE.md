...
Astro 6 + AstroPaper v6.1.0 静态博客，部署到 GitHub Pages。样式用 Tailwind v4（`@tailwindcss/vite`），代码高亮 shiki，站内搜索 pagefind，内容用 MDX。
...
- 开发：`pnpm dev`
- 构建：`pnpm build`（= `astro check && astro build`）
- 校验：`pnpm lint`（eslint）、`pnpm format:check`（prettier；`pnpm format` 可自动修复）
- 同步 content schema：`pnpm sync`

## 目录约定

- 博客文章：`src/content/posts/*.md(x)`；静态页：`src/content/pages/`（均为 content collection，路由自动生成）
...
- **组件是 `.astro`，不是 React/Vue**——仓库无前端框架依赖，禁止引入 React 或任意 UI 框架。
